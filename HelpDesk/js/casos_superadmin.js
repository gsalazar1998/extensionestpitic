$(function(){
   ...
   $.datepicker.setDefaults($.datepicker.regional['es']);
	...
	$("#sub_btn_ec").click(function(){
		//TReporteCasos.fnReloadAjax();
		$.ajax({
			url: "../php/reporteActividadesExtraCaso.php?opc=2",
			data: $("#rpt_extra_casos").serialize(),
			type: "post",
			success:function(data){
				$("#info_casoRPT_ec").html(data);
			},
			failure: function(){}
		});
		//$("#resultados").html("asdasdasd");
	...
	//tabla 7
	var casosVencidosT7; //el T7 del final es por tabla 7
	var asiSelectedT7 = [];
	
	//tabla 8 tab 9
	var autactecT8; //el T5 del final es por tabla 5
	var asiSelectedT8 = [];
	...
	$('#cnoasignados tbody tr ').live('click', function () {
		...
		$("#asignacaso_ventana").html(
			...
			"<optgroup label='SISTEMAS'>"+
				...
				"<option value='fvargas'>fvargas</option>"+
				"<option value='zsainz'>zsainz</option>"+
				"<option value='iaguiar'>iaguiar</option>"+
				"<option value='gesquivel'>gesquivel</option>"+
				"<option value='cquintero'>cquintero</option>"+
				"<option value='emonge'>emonge</option>"+
                                "<option value='ccovarrubias'>ccovarrubias</option>"+
				"<option value='eresendiz'>eresendiz</option>"+
				"<option value='hgonzalez'>hgonzalez</option>"+
				"<option value='cprado'>cprado</option>"+
				"<option value='marivera'>marivera</option>"+
                                "<option value='iperez'>iperez</option>"+
				"<option value='jfavila'>jfavila</option>"+
				"<option value='jfelix'>jfelix</option>"+
				"<option value='gsalazar'>gsalazar</option>"+
				"<option value='mrios'>mrios</option>"+
			"<optgroup label='COBRANZA'>"+
				"<option value='vocana'>vocana</option>"+
			"</optgroup>"+
			"<optgroup label='OPERACIONES'>"+
				"<option value='mabitia'>mabitia</option>"+
				"<option value='xilma'>xilma</option>"+
			"</optgroup>"+
	...
	//ventana de informacion del caso que se abrira en el caso de los reportes.
	$('#detallesReportes_ec').live('click',function(){
		$.ajax({
			url: "../php/casos_admin.php?tidf=13",
			data: "idcaso="+selectedRPT[0],
			type: "POST",
			success: function(data){
				//$("#asignacaso_ventana").dialog("close");
				$('#infoCasoReporte_ec').html(data);
				$('#infoCasoReporte_ec').dialog('open');
			},
			failure: function(){
				alert("Error en la peticion");
			}
...
/**************** COMIENZA LA TABLA 7 (Casos vencidos) *******************/

var casosVencidosT7 = $('#casosVencidos').dataTable({
	"bJQueryUI": true,
	"sPaginationType": "full_numbers",
	"bProcessing": true,
	"sAjaxSource": '../php/casos_admin.php?tidf=11', //aca me quede
	/*"fnRowCallback": function( nRow, aData, iDisplayIndex ) {
	if ( jQuery.inArray(aData[0], asiSelected) != -1 ){
				$(nRow).addClass('row_selected');
		}
...
// Evento de click 
$('#casosVencidos tbody tr ').live('click', function () {
	//$("#info_casoT7").html("<a href='#' id='casos_info_excepcion' class='ui-state-default ui-corner-all fuente'><span class='ui-icon ui-icon-arrowreturnthick-1-e'></span>Ver Detalles</a><p></p>");
	$("#info_casoT7").html("<a href='#' id='casos_info_linkT7' class='ui-state-default ui-corner-all fuente'><span class='ui-icon ui-icon-arrowreturnthick-1-e'></span>Ver Detalles</a></p>");
	var aData = casosVencidosT7.fnGetData(this );
	var iId = aData[0];
	
	$('#casosVencidos tbody tr ').removeClass("row_selected");
	$(this).addClass("row_selected");
	if ( jQuery.inArray(iId, asiSelected) == -1 )
	{
		asiSelectedT7[0] = iId;
...
$('#info_casoT7').live('click',function(){
	$.ajax({
		url: "../php/casos_admin.php?tidf=5",
		data: "idcaso="+asiSelectedT7[0],
		type: "POST",
		success: function(data){
			//$("#asignacaso_ventana").dialog("close");
			$('#infoExcepcion').html(data);
			$('#infoExcepcion').dialog('open');
		},
		failure: function(){
			alert("Error en la peticion");
		}
...
$("#infoExcepcion").dialog({
	autoOpen: false,
	width: 800,
	draggable: true,
	resizeable: false,
	buttons:{
		"Solicitar Excepcion": function(){
			$.ajax({
				url: "../php/casos_admin.php?tidf=12",
				data: "idcaso="+asiSelectedT7[0],
				type: "POST",
				success: function(data){
						if(data == "OK"){
							$("#msgDialog").html("Solicitud Registrada");
							$('#msgDialog').dialog('open');
						}
				},
				failure: function(){
...
/****************************/
		...
		/*********************** AUTORIZACION DE ACTIVIDADES EXTRA CASO ***/
		var autcasosec = $('#autactec').dataTable({
			"bJQueryUI": true,
			"sPaginationType": "full_numbers",
			"bProcessing": true,
			//"sAjaxSource": '../php/casos_admin.php?tidf=11',
			/*"fnRowCallback": function( nRow, aData, iDisplayIndex ) {
			if ( jQuery.inArray(aData[0], asiSelected) != -1 ){
						$(nRow).addClass('row_selected');
				}
...
$('#refresh_asignados').click(function(){
	...
	sinConfirmacion.fnReloadAjax();
	casosAiertosT5.fnReloadAjax();
	casosVencidosT7.fnReloadAjax();
})	
/********* ventana para iniciar revision y categorizar el caso (MIS CASOS ABIERTOS) *********************/
$("#inic_revision_ven").dialog({
		autoOpen: false,
		width: 330,
		draggable: true,
		resizeable: false,
		buttons:{
			"Aceptar": function(){
				$.ajax({
					url: "../php/marcarARevision.php",
					data: "idcaso="+$("#num_caso").val()+"&sistema="+document.iniciaRevision.sistemas.options[document.iniciaRevision.sistemas.selectedIndex].value,
					type: "POST",
					success: function(data){
						if(data == "OK"){
							$('#inic_revision_ven').dialog("close");
							$('#revision_div').html("EN REVISION");
							$('#msgDialog').html("Se ha marcado el caso como 'EN REVISION'");
							$('#msgDialog').dialog('open');
							$('#inic_revision_ven').dialog('close');
						}else{
							$('#msgDialog').html(data);
			...
			"Aceptar": function(){
				if($("#cerrarcasoSC").validationEngine('validate')){
					$.ajax({
						...
						data: "idcaso="+$("#num_caso").val()+"&finalComment="+$("#comentarioFinal").val()+"&oficina="+document.cerrarcasoSC.oficinasCierraCaso.options[document.cerrarcasoSC.oficinasCierraCaso.selectedIndex].value,
						type: "POST",
						success: function(data){
							if(data == "OK"){
