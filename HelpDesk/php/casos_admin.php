...
if(!isset($_SESSION['usuario'])){
	...
	switch($_GET["tidf"]){
		...
		case 11: echo obtenerCasosVencidos($_SESSION['usuario']); break;
		case 12: echo SolicitarExcepcionACaso($_POST['idcaso']); break;
		case 13: echo obtenerActividadesExtraCasoInfo($_POST['idcaso'], $_SESSION['usuario']); break;
	}
}
...
function obtenerInfoCaso($idCaso){
	...
	if($numero >0){
		...
		<tr>
			<td class='tableLabels'>Caso No.</td>
			<td class='tableLabels'>Fecha:</td>
			<td class='tableLabels'>Hora</td>
			<td class='tableLabels'>Inicio revision en:</td>
		</tr>
		<tr>
			<td>".$row['ID_CASO']."<input type='hidden' id='num_caso' name='num_caso' value='".$row['ID_CASO']."' /></td>
			<td>".substr($row['FECHA_APERTURA'],0,10)."</td>
			<td>".substr($row['FECHA_APERTURA'], -8)."</td>";
			if(isset($row['FEC_INICIO_REVISION'])){
				$nassign .= "<td><span id='revision_div'>".$row['FEC_INICIO_REVISION']."</span></td></tr>";
			}else{
				$nassign .= "<td align='center'><span id='revision_div'><input type='button' id='revision_btn' value='Iniciar Revision'></button></span></td></tr>";
			}
		$nassign .= 
		"
		<tr>
			<td class='tableLabels'>Abierto por: </td>
			<td class='tableLabels' >Oficina</td>
			<td class='tableLabels' colspan='2'>Depto.</td>
		</tr>
		<tr>
			<td>".$row['NOMBRE']." (".$row['USUARIO'].")</td><td>".$row['OFICINA']."</td>
			<td colspan='2'>".$depto['DEPARTAMENTO']."</td>
		<tr>
			<td colspan='2' class='tableLabels'>Problema: </td>
			<td class='tableLabels' colspan='2'>T&eacute;lefono</td>
		</tr>
		<tr>
			<td colspan='2'>".$row['PROBLEMA']."</td>
	...
	$nassign.= "<script>
		$(function() {
			...
			$( '#revision_btn' ).button();
			$( '#revision_btn' ).live('click', function(){
				$('#inic_revision_ven').dialog('open');
			});
...
function obtenerInfoCasoPM($idCaso){
	...
	if($numero >0){
		...
		<div id='accordionDescPM' class='fuenteTable'>
			<h3><a href='#'>Descripcion del problema</a></h3>
			<div id='descripcionInfoCaso'>
				<table>
				<tr><td>".nl2br($row['DESCRIPCION'])."</td></tr>
...
//echo 'xxxx<br>';
		...
		<div id='accordionDescRep' class='fuenteTable'>
			<h3><a href='#'>Descripcion del problema</a></h3>
			<div id='descripcionInfoCaso'>
				<table>
	...
	$nassign.= "<script>
		$(function(){
			...
			$( '#accordionAdjRep' ).accordion({
				collapsible: true,
				active: -1
			});
			
			$( '#accordionDescRep' ).accordion({
				collapsible: true,
			});
...
function obtenerActividadesExtraCasoInfo($idCaso, $usuario_sesion){
	$mysql = new MySQL();
	$mysql->connect(); 

	$query = "SELECT * FROM act_extra_casos WHERE ID_ACTIVIDAD = '".$idCaso."'";
	$result = $mysql->query($query);
	$numero = mysql_num_rows($result);
	if($numero >0){
		$respuestas = obtenerRespuestas($idCaso);
		...
		$nassign = "<table border='0' style='width:100%'>
		<tr><td class='tableLabels'>Actividad No.</td><td class='tableLabels'>Fecha:</td><td class='tableLabels'>Hora</td></tr>
		<tr><td>".$row['ID_ACTIVIDAD']."</td><td>".substr($row['FECHA'],0,10)."</td><td>".substr($row['FECHA'], -8)."</td></tr>
		<tr><td class='tableLabels'>Abierto por: </td><td class='tableLabels'>Oficina</td><td class='tableLabels'>Atendi&oacute;</td></tr>
		<tr><td>".$row['NOMBRE']." (".$row['USUARIO'].")</td><td>".$row['OFICINA']."</td><td>".$row['ATENDIDO_POR']."</td>
		</table>
		<div id='accordionDesc_ec' class='fuenteTable'>
			<h3><a href='#'>Descripcion de la Actividad</a></h3>
			<div id='descripcionInfoCaso'>
				<table>
				<tr><td>".$row['PROBLEMA']."</td></tr>
				</table>
			</div>
		</div><br />";
		if($usuario_sesion == "cmburboa"){
			$nassign .= "<input type='button' id='autorizarExtracaso' value='Autorizar'>
						<input type='button' id='rechazarExtracaso' value='Rechazar'>";
		}
	$nassign.= "<script>
		$(function(){		
			$( '#accordionDesc_ec' ).accordion({
				collapsible: true,
			});
			
			$( '#autorizarExtracaso' ).button();
			$( '#rechazarExtracaso' ).button();
			
			$( '#autorizarExtracaso' ).live('click', function(){
				alert('Autorizado');
			});
			
			$( '#rechazarExtracaso' ).live('click', function(){
				alert('Rechazado');
			});
			";
	$nassign.= "
...
function obtenerCasosVencidos($usuario){
	$mysql = new MySQL();
	$mysql->connect(); 
	...
	ESTADO_CASO,
	FECHA_CIERRE,
	TIMEDIFF(FECHA_CIERRE, FEC_INICIO_REVISION) AS HRS_TOTALES
	FROM casos WHERE ASIGNADO_A = '".$usuario."' AND ESTADO_CASO != 1 AND TIEMPO_VENCIDO = 1 ORDER BY ID_CASO DESC"; //A*/
	...
	IF(ISNULL(a.FECHA_CIERRE) = 1, '--', 
		  IF((SELECT COUNT(b.ID_CASO) FROM casos_autorizaciones b WHERE b.ID_CASO = a.ID_CASO AND b.FECHA_AUTORIZACION IS NOT null) > 0, 
			...
			CONCAT(TIMEDIFF(a.FECHA_CIERRE, a.FECHA_APERTURA)))) AS HRS_TOTALES
	FROM casos a WHERE ASIGNADO_A = '".$usuario."' AND ESTADO_CASO != 1 AND TIEMPO_VENCIDO = 1 ORDER BY ID_CASO DESC";
	$result = $mysql->query($query);
	$numero = mysql_num_rows($result);
	...
	if($numero >0){
		...
		while($row = mysql_fetch_array($result)){
			$mysql = new MySQL();
			$mysql->connect(); 
			$q = "SELECT tiempo_limite FROM cat_sistemas WHERE clave = '".$row['TIPO_PROBLEMA']."'"; //A
			$rs = $mysql->query($q);
			...
			$nassign .='["'.$row['ID_CASO'].'","'.$row['USUARIO'].'","'.addslashes($row['PROBLEMA']).'","'.$row['FECHA_APERTURA'].'","'.$row['FECHA_ASIGNACION'].'","'.$row['FEC_INICIO_REVISION'].'","'.$row['FECHA_CIERRE'].'", "'.$row['HRS_TOTALES'].'", "'.$renglon['tiempo_limite'].'"],';
		}
		$nassign = substr($nassign,0, -1)."]}";
	}else{
		$nassign = '{ "aaData": []}';
...
function SolicitarExcepcionACaso($idcaso){
	$mysql = new MySQL();
	$mysql->connect(); 

	$query = "UPDATE SISP.casos SET TIEMPO_VENCIDO = 2 WHERE ID_CASO = '".$idcaso."'";
	$result = $mysql->query($query);
	if($result){
		echo "OK";
	}
