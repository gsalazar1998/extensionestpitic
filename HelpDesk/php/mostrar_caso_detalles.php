	...
	if($numero >0){
		...
		<div id='accordionDesc' class='fuenteTable' style='width:100%'>
			...
			<div id='descripcion'>
				...
				$nassign.=">".$respuestas."</div>";
				if($_SESSION['usuario'] == $row['ASIGNADO_A'] || $_SESSION['usuario'] == $row['USUARIO'] || $_SESSION['usuario'] == $row['AUTORIZADO_POR']){
					$nassign .= "<input type='button' id='resp_btn' value='Agregar Comentario'></button></td></tr>";
				}else{
					$nassign .= "Usted no puede comentar en este caso debido a que no lo tiene asignado</td></tr>";
				}
				$nassign .= "</table>
			</div>
		</div>";
...
<html>
	...
	<body id="dt_example" class="fuente">
		<center><div style="width:70%;"><?php echo obtenerInfoCaso($_GET['idcaso']); ?></div></center>
...
<?php }else {
		echo "<script>window.location = 'index_m_caso_auto2.php?idcaso=".$_GET['idcaso']."'</script>";
	}
?>
