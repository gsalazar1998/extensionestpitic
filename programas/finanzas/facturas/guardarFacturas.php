...
if($tipo == "cbb"){
	...
	$fec_aprob = isset($_POST['fec_aprob']) ? $_POST['fec_aprob'] : "NULL";
	
	$query = "INSERT INTO CBB VALUES(null, '".$rfc."', '".$proveedor."', '".$folio_fac."', '".$num_sicofi."', '".$fec_aprob."',";
	$query.= "'".$folios_aut."', '".$serie."', NOW(), '".$_COOKIE['usuario_accesofin']."')";
	
	$rs = mysql_query($query) or die(mysql_error());
	if($rs){
		echo "factura guardada correctamente";
	}else{echo "no se grabo";}
}
...
//CFDI
	...
	//Acá se sube los archivos PDF
	$uploaddir = '/var/www/sistemas.tpitic.com.mx/programas/finanzas/facturas/facturas/';
	$pdfs = $_FILES['pdf']['name'];
	$uploadfile = $uploaddir.basename($pdfs);
	
	if (move_uploaded_file($_FILES['pdf']['tmp_name'], $uploadfile)) {
		echo "El archivo es v&aacute;lido y fue cargado exitosamente.\n";
	} else {
	...
	//termina la subida de PDFs
	/*print("<pre>");
	print_r($_FILES);
	print("</pre>");*/
	
	
	//Acá comienza para guardar XML's
	$uploaddir = '/var/www/sistemas.tpitic.com.mx/programas/finanzas/facturas/facturas/';
	$xmls = $_FILES['xml']['name'];
	$uploadfile = $uploaddir.basename($xmls);
	...
	if($xmls != "NULL" and $xmls != ""){
		...
		if ( $archivo != "none" ){
			...
			fclose($fp);
			/*$contenido = fopen($archivo, "rb") or exit("Unable to open file!");
			...
			if($contenido != ""){
				switch($tipo){
					case "cfd": $query = "INSERT INTO CFD VALUES (null, '".$rfc."', '".$proveedor."','".$folio_fac."','".$no_aprobacion."','".$ano_apro."','".$serie_cert."','".$serie."',NOW(),'cmburboa','".$contenido."', '".$pdfs_renamed."')"; break;
					case "cfdi": $query = "INSERT INTO CFDI VALUES (null, '".$rfc."', '".$proveedor."','".$folio_fac."','".$serie_cert_digital."','".$serie_sat."','".$fec_certificacion."','".$folio_fisca."','".$serie."',NOW(),'cmburboa','".$contenido."','".$pdfs_renamed."')"; break;
				}
				echo $query;
			}else{
